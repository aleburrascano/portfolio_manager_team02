...
def _close_by_date(asset_type: str, ticker: str) -> Dict[date, float]:
    ...
    One ticker whose history can't be fetched
    leaves a gap in the chart.
    """
...
def get_portfolio_performance(user_id: int, days: int = 365) -> Dict[str, Any]:
    """
    Calculate a user's portfolio value over the last (no. days) days.

    Each point carries three values: the assets at market
    value, the cash alongside them, and the net deposits
    they were bought with. (At each day)
    * Gain/loss is the difference between the last two values
    Args:
        user_id (int): The ID of the user.
        days (int): How far back to chart.

    Each point also carries `benchmarkValue`: what the same deposits, made
    on the same days, would be worth in a broad index fund / other investments.
...
def average_cost_basis(trades: List[dict]) -> Dict[str, Decimal]:
    ...
    A position closed out to zero resets, so buying back in starts
