...
import { useLiveFeeds, useQuoteConnection } from '../../hooks/realtime'
...
function Watchlist({
  ...
  useEffect(() => {
    ...
    async function loadWatchlist() {
      try {
        const entries = await fetchWatchlist(user.userId)
        if (cancelled) return
        setSaved(entries)
  ...
  return (
    <section className="card watchlist-card" aria-labelledby="watchlist-title">
